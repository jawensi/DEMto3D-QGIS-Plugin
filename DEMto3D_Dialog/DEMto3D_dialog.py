...
from qgis.PyQt.QtWidgets import QApplication, QDialog, QFileDialog, QMenu, QMessageBox
...
class DEMto3DDialog(QDialog, Ui_DEMto3DDialogBase):
    ...
    def __init__(self, iface: "QgisInterface"):
        ...
        menu.addSeparator()
        menu.addAction(
            self.tr("Export extension to GeoJSON"), self.exportExtensionToJSON
        )
        ...
        if parameters != None:
            ...
            if setting_file[0] != "":
                ...
                QMessageBox.information(
                    self, self.tr("Attention"), self.tr("Parameters exported")
                )
        else:
            QMessageBox.warning(
            ...
            with open(setting_file[0]) as json_file:
                try:
                    ...
                    if "borders" in parameters:
                        self.ui.borderModelLineEdit.setText(
                            str(round(parameters["borders"], 3))
                        )
                    ...
                    QMessageBox.warning(
                        self, self.tr("Attention"), self.tr("Wrong file")
                    )
    ...
    def exportExtensionToJSON(self):
        try:
            ...
            setting_file = QFileDialog.getSaveFileName(
                self, self.tr("Export extension to GeoJSON"), file_name, "*.geojson"
            )
            if setting_file[0] != "":
                self.lastSavingPath = os.path.dirname(setting_file[0]) + "//"
                ...
                QMessageBox.information(
                    self, self.tr("Attention"), self.tr("Extension exported")
                )
        except:
            QMessageBox.warning(
                self, self.tr("Attention"), self.tr("Fill the data correctly")
    ...
    def exportExtensionToKML(self):
        try:
            ...
            setting_file = QFileDialog.getSaveFileName(
                self, self.tr("Export extension to KML"), file_name, "*.kml"
            )
            if setting_file[0] != "":
                self.lastSavingPath = os.path.dirname(setting_file[0]) + "//"
                # Specify the geometry type
                ...
                crs_source = (
                    self.layer.crs()
                )  # Original layer's coordinate reference system
                crs_target = QgsCoordinateReferenceSystem("EPSG:4326")  # EPSG:4326
                transform = QgsCoordinateTransform(
                    crs_source, crs_target, QgsProject.instance()
                )
                # Apply transformation to each point of the polygon
                transformed_points = [
                    [transform.transform(pt) for pt in ring] for ring in points
                ]
    ...
    def upload_extent(self):
        try:
            ...
            QMessageBox.warning(
                self, self.tr("Attention"), self.tr("Value entered incorrect")
            )

    def upload_extent_fromWH(self):
        if (
            self.ui.WidthGeoLineEdit.text() == ""
            or self.ui.HeightGeoLineEdit.text() == ""
        ):
            return
        if (
            self.roi_x_min == 0
            and self.roi_y_min == 0
            and self.roi_x_max == 0
            and self.roi_y_max == 0
        ):
            return
        try:
            widthGeo = float(self.ui.WidthGeoLineEdit.text())
            heightGeo = float(self.ui.HeightGeoLineEdit.text())
            p2x, p2y = getPolarPoint(
                self.roi_x_min, self.roi_y_min, self.rect_Params["rotation"], widthGeo
            )
            p1x, p1y = getPolarPoint(
                p2x, p2y, self.rect_Params["rotation"] + math.pi * 0.5, heightGeo
            )
    ...
    def paint_model_division(self):
        ...
        if y_models > 1:
            ...
            for i in range(1, y_models):
                p1 = getPolarPoint(
                    points[0][0], points[0][1], custRot, model_height * i
                )
                p2 = getPolarPoint(
                    points[1][0], points[1][1], custRot, model_height * i
                )
        ...
        if self.units == Qgis.DistanceUnit.Degrees:
            ...
            if self.layer.crs().mapUnits() == Qgis.DistanceUnit.Degrees:
                ...
                cell_size_m = (
                    ...
                    * math.pi
                    / 180
                    * math.cos(self.raster_y_max * math.pi / 180)
                    * 6371000
                )
    ...
    def upload_size_from_height(self):
        try:
            ...
            QMessageBox.warning(
                self, self.tr("Attention"), self.tr("Define print extent")
            )
            self.ui.HeightLineEdit.clear()
        except ValueError:
            QMessageBox.warning(
    ...
    def upload_size_from_width(self):
        try:
            ...
            QMessageBox.warning(
                self, self.tr("Attention"), self.tr("Define size model")
            )
            self.ui.WidthLineEdit.clear()
        except ValueError:
            QMessageBox.warning(
    ...
    def get_height_model(self):
        if (
            self.ui.BaseHeightLineEdit.text() == ""
            or self.ui.BaseHeightLineEdit.text() == "-"
        ):
            return
        try:
            ...
            h_model = round(
                (self.z_max - z_base) / self.scale * 1000 * self.z_scale + base_model, 1
            )
